 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a negative and positive") {
    assert(sum(List(-1,1)) === 0)
  }
  test("sum of a negatives") {
  ...
  test("max of zeroes") {
    assert(max(List(0,0,0,0,0,0)) === 0)
  }
  test("max of negatives and positives") {
    assert(max(List(-1,-2,3,6)) === 6)
  }
  test("max of negatives") {
    assert(max(List(-1,-2,-3,-5)) === -1)
  }
  test("max of dupes") {
